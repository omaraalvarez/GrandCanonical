...
function Mezei(ChemPot::Float64, V::Float64, T::Float64, R_Cut::Float64 = 3.)
    ...
    for i = 1:MC_Steps
        ...
        if RN == 2
            N_Insertion += 1;
            Pc_Grid, Pc_Grid_Sum, Pc_Grid_N = Grid_Excluded_Volume(L, Pc_Grid, Pc_Grid_Sum, Pc_Grid_N, x, y, z)
    ...
    for i = 1:length(Pc_Array)
        println(Pc_File, "$i\t$(round(Pc_Array[i], digits = 6))\t$(round(Pc_Grid_Array[i], digits = 6))")
    end
    close(Pc_File)
    ...
    Energy_Histogram = histogram(Energy_Array, bins = 20, legend = false, xlabel = "Energy [Unitless]", ylabel = "Frequency", size = [1200, 800])
    ...
    Pressure_Plot = plot(Pressure_Array, legend = false, xlabel = "Measurements", ylabel = "Pressure [Unitless]", width = 3, size = [1200, 800])
    hline!([mean(Pressure_Array)], color = :black, width = 2, linestyle = :dash)
    savefig(Pressure_Plot, "$Output_Route/Pressure")
    Pressure_Histogram = histogram(Pressure_Array, bins = 20, legend = false, xlabel = "Pressure [Unitless]", ylabel = "Frequency", size = [1200, 800])
    ...
    Density_Histogram = histogram(Density_Array, bins = 20, legend = false, xlabel = "Density [Unitless]", ylabel = "Frequency", size = [1200, 800])
...
function Grid_Excluded_Volume(L::Float64, Pc_Grid::Dict{Int64, Float64}, Pc_Grid_Sum::Dict{Int64, Float64}, Pc_Grid_N::Dict{Int64, Int64}, x::Array{Float64, 1}, y::Array{Float64, 1}, z::Array{Float64, 1})
    Grid = 10;
    Delta = L / (Grid + 1);
